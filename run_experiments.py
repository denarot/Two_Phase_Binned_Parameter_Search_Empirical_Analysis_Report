...
def run_bidirectional_validation() -> Dict[str, Any]:
    ...
    for dataset_name in get_dataset_names():
        ...
        for seed in range(42, 52):
            ...
            reverse = two_phase_search_reverse(X_train, y_train, k_min=10, k_max=1000, epsilon=0.001, window_size=5, random_state=seed)
            diff = abs(forward.k_hat - reverse.k_hat) / max(forward.k_hat, reverse.k_hat)
            dataset_results.append({
                'forward_k': forward.k_hat,
                'reverse_k': reverse.k_hat
            })
